...
- **5**: 같은 `aws/spans` 의 `cache_read_input_tokens / input_tokens × 100` 비율. **전 모델 합산 값**이라 이 워크샵 구성에서는 **40~60% 가 정상 범위**입니다 (`cache_config` 를 켠 Orchestrator(Sonnet) 만 캐시를 쓰고, Haiku 서브에이전트 3개는 캐시가 없어 분모를 키웁니다). 평소 수준에서 **급락**하면 system prompt 가 의도치 않게 가변화된 신호로, 비용·지연이 함께 증가합니다
...
| 5. Prompt Cache Hit Ratio | **40~60%** (Orchestrator 만 캐시 사용 — 아래 설명 참고). 0% 면 cache 미동작 |
...
| (c) Cache Hit Ratio ↓ | 평소 대비 **급락** 후 30분 지속 (예: 45% → 10%) | system prompt 회귀로 비용 급증 신호. 절대값 60% 기준은 이 구성에 맞지 않음 |
