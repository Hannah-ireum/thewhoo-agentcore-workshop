...
- **5**: 같은 `aws/spans` 의 `cache_read_input_tokens / input_tokens × 100` 비율. 60% 이하로 떨어지면 system prompt 가 의도치 않게 가변화된 신호로, 비용·지연이 함께 증가합니다
...
| 위젯 | 결과 |
|---|---|
| 1. 호출량 / 에러 / Throttle | Invocations ~45, **UserErrors ~5** (의도된 에러로 잡힘) |
| 2. Latency P50/P95/P99 | 정상 호출 ~40개로 분포 풍부 |
| 3. Session 수 | **10** |
| 4. 모델별 토큰 사용량 | Sonnet (orchestrator) + Haiku (sub-agent) 누적 |
| 5. Prompt Cache Hit Ratio | 60%+ (멀티턴이라 동일 session 내 hit 많음) |
| 7. Runtime 에러 로그 tail | 평소 비어있는 게 정상. 위젯 1 의 UserErrors 는 Runtime API frontend 의 schema 위반까지 잡지만, 이 위젯은 **컨테이너 내부에서 Python Exception 이 raise** 될 때만 row 가 추가됩니다. 워크샵 agent 는 도메인 에러도 LLM 이 정상 응답으로 처리하므로 의도된 schema 위반 invoke 만으로는 채워지지 않습니다 — "no news = good news" 위젯. |
...
| (c) Cache Hit Ratio ↓ | 60% 이하 30분 지속 | system prompt 회귀로 비용 급증 신호 |
...
| 증상 | 원인 / 해결 |
|---|---|
| 위젯 1·2·3 이 비어있음 | metric 은 첫 트래픽 후 5-10분 지연 후 publish 됩니다. 2단계의 트래픽 발생 명령을 한 번 더 돌리고 10분 대기. 검증: `aws cloudwatch list-metrics --namespace AWS/Bedrock-AgentCore --region us-east-1` 결과에 metric 이 나오면 publish 시작된 것. |
