...
## 🖥️ 고객과 콘솔에서 함께 볼 세 화면

Day 2 의 목표는 "운영하면서 확인할 수 있는가" 입니다. 아래 세 화면을 순서대로 보여주면 **관측 → 진단 → 품질** 흐름이 그대로 드러납니다. 실제 완주에서 세 화면 모두 데이터가 채워지는 것을 확인했습니다.

| 순서 | 화면 | 무엇을 보여주나 | 콘솔 경로 |
|---|---|---|---|
| ① | **CloudWatch Dashboard** | 지금 전체가 정상인가 — 호출량·지연·토큰·캐시 | CloudWatch → Dashboards → `thewhoo-chat-runtime` |
| ② | **GenAI Observability** | 이 한 건에서 무슨 일이 있었나 — span 트리 | CloudWatch → **GenAI Observability** → Agents → Sessions |
| ③ | **평가 리포트** | 답변 품질이 회귀했나 — 골든셋 매트릭스 | `eval-report.py` 로 만든 HTML (Lab 8) |

▶ **Dashboard URL 을 바로 얻기**

```bash
ACC=$(aws sts get-caller-identity --query Account --output text)
echo "https://${AWS_REGION}.console.aws.amazon.com/cloudwatch/home?region=${AWS_REGION}#dashboards:name=thewhoo-chat-runtime"
echo "https://${AWS_REGION}.console.aws.amazon.com/cloudwatch/home?region=${AWS_REGION}#gen-ai-observability"
```

> **고객 시연 순서 팁** — ①에서 "호출 5건, P95 지연, 캐시 19%" 같은 **숫자**를 먼저 보여주고, ②에서 그 숫자가 어디서 나왔는지 **span 트리로 내려가고**, ③에서 "그래서 품질은 유지되고 있다" 를 보여주는 흐름이 가장 설득력 있습니다.

### ⭐ 캐시가 동작한다는 증거를 숫자로 보여주기

위젯 4(모델별 토큰)에서 실측된 값입니다:
...
> **Sonnet 의 `input_tokens` 가 69 인 것이 핵심입니다.** 공식 문서대로 `inputTokens` 는 **캐시되지 않은 토큰만** 세므로, 이 숫자가 작다는 건 prompt caching 이 실제로 작동한다는 직접적인 증거입니다. 고객에게 "캐시가 켜져 있다" 를 말로 설명하는 대신 이 두 줄을 보여주세요.
>
> 전 모델 합산 캐시 히트율은 **19.55%** 로 측정됐습니다 — 위젯 5 의 20~40% 범위 안이고, Haiku 3개가 분모를 키우기 때문입니다.
