...
## 📊 대시보드 읽는 법 — 위젯별로 무엇을 말해주나

시뮬레이션이 끝나면 이 순서로 보면 됩니다. **"정상이 어떤 모습인가" 를 먼저 익히는 것**이 목적입니다 — 그래야 나중에 이상을 알아볼 수 있습니다.
...
**우측 상단 시간 범위를 `3h` 로 맞추세요** — 기본값(1h)이면 방금 트래픽이 화면 밖으로 밀려 빈 것처럼 보입니다.

### ① 호출량 / 에러 / Throttle — "요청이 들어오고 있나"

`Invocations` 선이 올라가 있으면 트래픽이 도달한 것입니다. **에러 3종은 0 이 정상** 입니다 (위의 설명 참고 — 잘못된 입력도 `app.py` 가 200 으로 안내 문구를 돌려주기 때문).

> 여기서 볼 것: **에러가 0 이라는 사실 자체.** 운영에서 이 위젯이 가장 먼저 보는 화면이고, 선이 올라가면 즉시 ②·⑦로 내려갑니다.

### ② Latency P50 / P95 / P99 — "느려지고 있나"

**P50 은 평범한 사용자 체감, P99 는 가장 느린 1%** 입니다. 둘의 간격이 벌어지면 "대부분은 빠른데 일부가 심하게 느리다" 는 뜻입니다 — 이 에이전트에서는 서브에이전트를 몇 개 호출했는지, Gateway 도구를 몇 번 탔는지에 따라 갈립니다.

> ⚠️ **선이 중간중간 끊겨 보이는 것은 정상입니다.** CloudWatch 는 **데이터가 없는 구간을 잇지 않습니다.** 워크샵 트래픽은 연속적이지 않아서, 호출이 없던 시간 구간마다 선이 비어 세로 빗금처럼 보입니다. **장애가 아니라 "그 시간엔 호출이 없었다" 는 뜻입니다.**
>
> 실서비스는 트래픽이 끊기지 않아 이 현상이 사라집니다. 워크샵에서 촘촘한 끊김을 줄이려면 시간 범위를 넓히세요(`3h` → `12h`) — 구간이 커져 빈칸이 메워집니다.

### ③ Session 수 — "몇 명이 쓰고 있나"

Invocations 와 나란히 보세요. **Sessions 는 적고 Invocations 는 많다** = 한 사람이 여러 턴 대화했다는 뜻입니다. 시뮬레이션은 10명 × 4턴이라 이 패턴이 나옵니다.

### ④ 모델별 토큰 사용량 — "돈이 어디서 나가나"

**비용의 직접적인 근거**입니다. Sonnet(Orchestrator)과 Haiku(서브에이전트)가 나뉘어 보입니다. Haiku 쪽 토큰이 훨씬 많은 것이 정상이고 의도된 설계입니다 — 무거운 작업을 값싼 모델에 넘겼다는 증거입니다.

### ⑤ Prompt Cache Hit Ratio — "캐시가 듣고 있나"

**20~40% 면 정상** 입니다 (실측 29%). Orchestrator 만 캐시를 쓰고 Haiku 3개는 최소 토큰(4,096)에 못 미쳐 캐시가 안 잡히므로, 100% 가 될 수 없는 구조입니다.

> 여기서 볼 것: **평소 수준을 기억해 두는 것.** 이 값이 갑자기 **급락**하면 system prompt 에 시각·랜덤값 같은 가변 요소가 들어가 캐시가 깨졌다는 신호입니다.

⭐ **캐시가 동작한다는 증거를 숫자로** — 위젯 ④에서 실측된 값입니다:

| 모델 | `input_tokens` | `cache_read` | 해석 |
|---|---|---|---|
| Sonnet 4.6 (Orchestrator) | **69** | **16,393** | 캐시가 입력을 거의 전부 흡수 |
| Haiku 4.5 (서브에이전트) | 71,631 | 0 | 캐시 미적용 (최소 토큰 미달) |

**Sonnet 의 `input_tokens` 가 69 인 것이 핵심입니다.** `inputTokens` 는 **캐시되지 않은 토큰만** 세므로(공식 문서), 이 숫자가 작다는 것이 prompt caching 이 실제로 작동한다는 직접 증거입니다.

### ⑥ CPU / Memory — "자원이 모자라나"

**최대 60분 지연**되는 vended metric 이라 워크샵 중에는 비어 있는 것이 정상입니다. 실서비스에서는 OOM·throttle 의 조기 신호이자 청구 추정의 근거가 됩니다.

### ⑦ Runtime 에러 로그 tail — "코드가 터졌나"

**비어 있는 것이 정상입니다.** 컨테이너 안에서 Python 예외가 raise 될 때만 줄이 추가되는 "no news = good news" 위젯입니다. 여기에 뭔가 보이면 ②의 GenAI Observability 로 내려가 해당 trace 를 찾습니다.

## ⏱️ 위젯이 비어 보일 때 — 지연 경로가 세 갈래입니다

"invoke 했는데 대시보드가 안 변한다" 는 가장 흔한 질문입니다. 원인은 **경로별 지연이 다르기 때문**이고, 그중 하나는 지연이 아니라 **콘솔 설정 문제**입니다.

| 위젯 | 데이터 경로 | 반영 시점 | 근거 |
|---|---|---|---|
| 1·2·3 (호출량·Latency·Sessions) | `AWS/Bedrock-AgentCore` native metric | **1분** 배치 | 공식: *"Runtime metrics are batched at one minute intervals"* |
| 4·5 (토큰·캐시) | `aws/spans` → Logs Insights | **2-5분** (배포 직후 첫 trace 는 ~10분) | Transaction Search 인덱싱 |
| 6 (CPU·Memory) | vended metric | **최대 60분** | 공식: *"Resource usage data may be delayed by up to 60 minutes"* |
| 7 (에러 로그) | 런타임 log group | 수십 초 | 애플리케이션 로그 |

즉 **위젯 1-3 은 1분 뒤면 나와야 정상**입니다. 안 나오면 지연이 아니라 다른 문제입니다.

### ⚠️ 가장 흔한 착시 — 시간 범위를 좁게 두면 빈 화면이 됩니다

실측으로 확인한 현상입니다. 같은 metric 을 조회 범위만 바꿔 요청했더니:

```
StartTime = now-3h,  Period=60   →  데이터포인트 0개   ← "비어 보임"
StartTime = now-24h, Period=60   →  데이터포인트 4개   ← 00:29~00:32 각 1건
```

**데이터는 처음부터 있었습니다.** CloudWatch 는 조회 범위와 period 조합에 따라 집계 결과를 돌려주지 않을 수 있습니다. 콘솔에서도 같습니다 — 우상단 시간 선택기가 기본 3시간이면 방금 찍힌 1분 단위 포인트가 안 보일 수 있습니다.

▶ **참가자에게 시킬 것** — 대시보드가 비어 보이면 **먼저 이것부터**:

1. 우상단 시간 범위를 **`1h`** 로 바꾸고 (기본값 말고 명시적으로 선택)
2. 위젯 우상단 `⋮` → **Refresh**
3. 그래프 하단 범례에서 metric 이름이 회색이면 클릭해 활성화

▶ **CLI 로 데이터 존재를 직접 확인** (콘솔 설정 문제인지 판별)
...
AGENT=$(echo "$RID" | sed 's/-[^-]*$//')

aws cloudwatch get-metric-statistics \
  --namespace AWS/Bedrock-AgentCore --metric-name Invocations \
  --dimensions Name=Resource,Value="arn:aws:bedrock-agentcore:${AWS_REGION}:${ACC}:runtime/${RID}" \
               Name=Operation,Value=InvokeAgentRuntime \
               Name=Name,Value="${AGENT}::DEFAULT" \
...
> **`--start-time` 을 24시간으로 넉넉히 잡는 게 핵심입니다.** 여기서 데이터가 나오면 metric 은 정상이고 **콘솔 표시 설정 문제**입니다. 여기서도 0이면 그때 지연·dimension 을 의심하세요.

> **진행 팁** — 대시보드를 만든 직후 바로 열지 말고, `simulate-traffic.sh --quick` 을 돌려 두고 **Lab 8 을 먼저 진행**한 뒤 돌아오세요. 그 사이 1-3 은 확실히 채워지고 4-5 도 인덱싱이 끝납니다. 위젯 6(CPU·Memory)은 워크샵 시간 안에 안 나올 수 있고 **그게 정상**입니다.
...
RUNTIME_ID=$(aws bedrock-agentcore-control list-agent-runtimes \
  --region us-east-1 \
  --query "agentRuntimes[?contains(agentRuntimeName, 'hewhoo')].agentRuntimeId | [0]" \
  --output text)
...
Slack 으로 보내려면 SNS → **Amazon Q Developer in chat applications** (구 AWS Chatbot) → Slack workspace 연결이 필요합니다 ([Slack 설정 가이드](https://docs.aws.amazon.com/chatbot/latest/adminguide/slack-setup.html)). 서비스 이름이 바뀌었지만 콘솔·문서 경로는 여전히 `chatbot` 입니다.
