...
AgentCore Memory 는 **계정 레벨 리소스 1개**입니다. 그 안에 여러 **strategy** 를 붙여 "무엇을 어디에 저장할지" 를 선언적으로 정의합니다. 챗봇 세션에서 `create_event()` 로 턴을 기록하면, AgentCore 가 **백그라운드에서 strategy 기준으로 장기 정보를 추출** 하고, 나중에 `retrieve_memories(namespace=...)` 로 꺼내 씁니다.
...
- **retention**: strategy 별로 데이터 만료 기간 설정 가능 (AgentCore Memory 콘솔)
...
- `session_id` 는 Summary strategy 에서만 씁니다. Preference / Semantic 은 세션 넘어 누적.
- `messages` 는 반드시 `USER` / `ASSISTANT` 쌍으로. 장기 추출 엔진이 이 구조로 "사용자 발화에서 선호 뽑기" 를 수행.
...
- `namespace` 는 **정확 prefix 매칭**. placeholder 를 치환한 전체 경로를 그대로 넣어야 합니다.
...
데이터가 적으면 `data/` 에 세럼을 추가하고 `scripts/reindex-kb.sh w001` 로 재색인하면 됩니다.
...
- **반환 개수 제한**: default 는 상위 5-10개. 토큰 아끼려면 `top_k=3` 식으로 제한
...
출력 예:
```
  summaryMemoryStrategy                    SessionSummary
  userPreferenceMemoryStrategy             UserPreference
  semanticMemoryStrategy                   SemanticFacts
  episodicMemoryStrategy                   EpisodicInteractions
```
...
| `Unknown parameter ... "reflection"` / `"namespaceTemplates"` | 실제 boto3 API 파라미터는 `reflectionConfiguration` 과 `namespaces` 입니다 (AWS devguide 의 일부 예제 문구와 다름). boto3 service model 이 진실 |
