...
| `thewhoo-get-promotion-<pid>` | `category`, `product_id` | 진행 중인 프로모션 | 할인/증정/쿠폰 조회 |

> 표의 파라미터는 각 `openapi.json` 의 입력 스키마와 일치합니다(대조 확인). `recommend_products` 의 `skin_type` 과 `check_stock` 의 `product_id` 만 **required** 이고 나머지는 선택입니다 — 그래서 `product_search` 는 인자 없이 불러도 전체 목록이 나옵니다.
...
### ⚠️ Gateway → Lambda 호출 계약 — 기존 Lambda 를 붙일 때 반드시 알아야 합니다

기존 API Gateway 용 Lambda 를 그대로 target 으로 등록하면 **동작하지 않습니다.** 공식 문서가 이 점을 명시합니다:
...
**event 는 `inputSchema` 의 properties 를 그대로 담은 flat dict 입니다.** API Gateway 처럼 `queryStringParameters` 나 `body` 로 감싸지 않습니다:

```python
# Gateway 가 보내는 event (공식 문서 예시)
{ "keywords": "wireless headphones", "category": "electronics" }
```

**도구 이름은 event 가 아니라 context 로 옵니다.** 그리고 `___` prefix 가 붙어 있어 직접 벗겨야 합니다:
...
context 에는 이 6개가 들어옵니다 — `bedrockAgentCoreToolName`, `bedrockAgentCoreMessageVersion`, `bedrockAgentCoreAwsRequestId`, `bedrockAgentCoreMcpMessageId`, `bedrockAgentCoreGatewayId`, `bedrockAgentCoreTargetId`.

> **워크샵 Lambda 는 어떻게 되어 있나** — `lambdas/*/app.py` 의 `_extract_params()` 가 `queryStringParameters` → `body` → **`event` 자체** 순서로 확인합니다. 마지막 fallback 덕분에 Gateway 의 flat dict 도 그대로 받습니다. 즉 Gateway 경유와 직접 호출(`aws lambda invoke`) 양쪽에서 동작하도록 만든 구조입니다.
>
> 함수가 도구 하나만 담당하므로 `bedrockAgentCoreToolName` 을 읽지 않아도 됩니다. **한 Lambda 가 여러 도구를 처리하려면** 위 boilerplate 처럼 context 에서 도구 이름을 꺼내 분기해야 합니다.
>
> 반환값은 워크샵 코드가 `{"statusCode": 200, "body": "<json 문자열>"}` 형태인데, 공식 요구사항은 *"must return a valid JSON response that can be parsed by the gateway"* 뿐입니다. 실측(Lab 3 도구 호출 출력)에서는 모델에게 **내부 JSON 내용이 전달**됩니다.
...
**설정 시점**: 이 워크샵은 `create-gateway.py` 가 **생성 시점에** `searchType=SEMANTIC` 으로 만듭니다. 그게 가장 확실한 경로입니다.

> API 상으로는 `UpdateGateway` 도 `protocolConfiguration.searchType` 을 받습니다 (botocore service model 확인). 즉 "생성 후 절대 변경 불가" 는 아닙니다. 다만 `UpdateGateway` 는 **부분 갱신이 아니라 지정한 필드로 덮어쓰는** 형태이고 `name`·`roleArn`·`authorizerType` 이 필수라, 기존 설정을 그대로 다시 넘기지 않으면 다른 설정이 초기화될 위험이 있습니다. 공식 문서에도 searchType 변경 절차에 대한 별도 안내가 없습니다.
>
> **Knowledge Base 와 대비해서 기억하세요** — KB 는 공식 문서가 *"You can't change the `knowledgeBaseConfiguration`"* 라고 **명시적으로 금지**합니다(변경하려면 재생성). Gateway 는 그런 금지 문구가 없습니다. AWS 리소스마다 "Update 로 바꿀 수 있는 범위" 가 다르니, 재생성이 필요한지는 각 API 문서에서 확인하는 습관이 중요합니다.
>
> 따라서 실무 권장은 **생성 시점에 정하기**입니다. 이미 만든 Gateway 를 바꿔야 한다면 `get-gateway` 로 현재 설정을 모두 읽어 그대로 포함해 `update-gateway` 를 호출하고, 반드시 target·도구 목록이 유지됐는지 확인하세요.
...
- **Gateway 관련 공식 quota** ([Quotas](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/bedrock-agentcore-limits.html#gateway-quotas)) — 도메인 분리 (예: 상품/주문/CS) 별로 Gateway 여러 개 두는 게 관리에 유리합니다.
...
| target / Gateway | 100 |
| tool / target | 1,000 |
| inline schema 크기 | 1 MB (S3 스키마는 10 MB) |
| 도구 이름 길이 | 256자 |
| tool-call·tool-list rate | Gateway·계정 각 200 TPS |
| **search 기반 tool-call rate** | **분당 25회** |
| Gateway invocation timeout | 15분 |

> ⚠️ **search 호출은 분당 25회**입니다. 여러 참가자가 같은 계정에서 Semantic Tool Search 를 반복 호출하면 throttling 이 발생할 수 있습니다.
...
Gateway 가 SEMANTIC 으로 생성되지 않은 상태입니다. 먼저 현재 값을 확인합니다.
...
`SEMANTIC` 이 출력되지 않으면, 워크샵에서는 **Gateway 를 지우고 `scripts/create-gateway.py` 를 다시 실행**하는 게 가장 빠릅니다 (target 4개까지 함께 재생성됩니다). 이 스크립트는 처음부터 SEMANTIC 으로 만듭니다.

> `update-gateway` 로 `searchType` 만 바꾸는 것도 API 상 가능하지만, 필수 필드(`name`·`roleArn`·`authorizerType`)와 기존 `protocolConfiguration` 을 모두 다시 넘겨야 해서 실습 중에는 오히려 실수하기 쉽습니다. 재생성이 안전합니다.
