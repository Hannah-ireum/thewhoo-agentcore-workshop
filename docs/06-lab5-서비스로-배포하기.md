...
>
> 이전 버전 워크샵은 Python `bedrock-agentcore-starter-toolkit` 을 썼지만, AWS 가 지원 종료를 공지했고 신규 기능은 새 CLI 에만 추가됩니다. 두 CLI 의 차이와 기존 프로젝트 이전 방법은 [새 CLI 이전 가이드](새-cli-이전-가이드.md) 를 참고하세요.
>
> **추가 전제조건** (Code Editor 에는 이미 준비돼 있습니다)
>
> | 항목 | 확인 명령 | 필요 버전 |
> |---|---|---|
> | Node.js | `node --version` | 20 이상 |
> | npm | `npm --version` | Node 에 포함 |
>
> `cdk bootstrap` 은 **직접 하지 않아도 됩니다** — 첫 `agentcore deploy` 가 필요하면 자동으로 처리합니다.
...
```json
{
  ...
  "envVars": {
    "KB_ID": "<본인 KB_ID>",
    "AGENTCORE_MEMORY_ID": "<본인 MEMORY_ID>",
    "AGENTCORE_GATEWAY_URL": "<본인 GATEWAY_URL>",
...
| `envVars` | 컨테이너 환경변수 | 구 CLI 의 `--env` 를 대체. KB·Memory·Gateway ID 를 여기서 주입 |
...
| invoke 시 `ThrottlingException` / `TooManyRequestsException` | Runtime 계정 기본 한도입니다. us-east-1 기준 동시 세션 5,000개 · 신규 세션 25개/초(CodeZip) · invoke 200건/초. 여러 PID 가 계정을 공유하거나 부하를 줄 때 나타납니다. Service Quotas 에서 증가 요청하거나 병렬 요청 수를 줄이세요 |
