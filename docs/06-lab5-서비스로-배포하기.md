...
agentcore --version          # 0.27.x 이상이 나오면 정상
```
...
python3 - <<PY
import json, pathlib
p = pathlib.Path("agentcore/agentcore.json")
d = json.loads(p.read_text())
d["runtimes"][0]["executionRoleArn"] = "arn:aws:iam::${ACC}:role/thewhoo-agent-role-w001"
p.write_text(json.dumps(d, indent=2, ensure_ascii=False))
print("executionRoleArn 설정 완료")
PY

agentcore validate     # Valid 확인
```

> `set-agentcore-config.py` 는 이 값을 자동으로 넣어 줍니다 — 스크립트를 썼다면 이 단계는 이미 끝났습니다. 손으로 `agentcore.json` 을 편집했다면 반드시 확인하세요.
>
...
### 🚨 Code Editor 에서 `xdg-open ENOENT` 가 나면
...
Code Editor 는 **원격 리눅스 컨테이너**입니다. 브라우저를 열 프로그램(`xdg-open`)이 없어서 마지막에 실패합니다. CLI 는 **서버를 먼저 띄운 뒤** 브라우저를 열려 하므로, **서버 자체는 떠 있습니다.**

> ⚠️ **`Chat UI: http://localhost:8081` 을 브라우저 주소창에 그대로 넣으면 안 됩니다.**
> 그 `localhost` 는 **Code Editor 컨테이너 안**을 가리킵니다. 여러분 노트북의 브라우저에서는 전혀 다른 곳(내 PC)을 찾으므로 열리지 않습니다.
> 원격 컨테이너의 포트를 보려면 **아래 포트 포워딩**이 필요합니다.

▶ **방법 A — 터미널만으로 (가장 간단, 권장)**

웹 화면 없이 확인만 하면 됩니다. `Ctrl+C` 로 끄고:
...
# 터미널 ② — Terminal → New Terminal 로 새로 열고
cd ~/thewhoo-agentcore-workshop
agentcore dev "천기단 화현 크림 성분 알려줘"
```

터미널 ②에 답변이 출력되면 성공입니다. **이걸로 3단계는 끝입니다.**

> `--no-browser` 는 쓰지 마세요 — Code Editor 에서 `Error: This command requires an interactive terminal` 로 실패합니다 (실측).

▶ **방법 B — 웹 inspector 를 꼭 봐야 한다면 (포트 포워딩)**

`agentcore dev` 를 그대로 켜 둔 상태에서 (`Ctrl+C` 금지), Code Editor **화면**에서:

1. 좌측 사이드바 하단 **PORTS** 탭
   (안 보이면 `Ctrl+Shift+P` → `Ports: Focus on Ports View`)
2. **Forward a Port** → 터미널에 나온 포트(예: `8081`) 입력 후 Enter
3. 목록에 생긴 행의 **지구본 아이콘 (Open in Browser)** 클릭

Code Editor 가 만들어 주는 주소는 이런 형태입니다:

```
https://<studio-id>.studio.<region>.sagemaker.aws/codeeditor/default/ports/8081/
```

> **반드시 Code Editor 를 열어 둔 그 브라우저에서** 클릭하세요. SageMaker 세션 인증이 필요해서 다른 브라우저나 외부에서 접근하면 **403** 이 됩니다.
>
> 포트 번호는 실행마다 바뀔 수 있습니다 — 터미널의 `Chat UI:` 줄에 나온 번호를 쓰세요.
> `agentcore dev` (플래그 없음) = 웹 UI **8081**, `agentcore dev --logs` = API **8080** 입니다.
>
> **포워딩이 안 되거나 시간이 걸리면 방법 A 로 넘어가세요.** 웹 inspector 는 이 워크샵에서 필수가 아닙니다.

### 이 단계에서 무엇을 확인하면 되나

`agentcore dev` 의 목적은 **배포 없이 코드가 도는지 보는 것**입니다. 아래 두 가지만 확인하면 충분하고, 그다음 4단계 배포로 넘어갑니다.

1. 터미널 ①에 `Application startup complete.` 가 떴다 → **서버 기동 성공**
2. 터미널 ②에서 질문에 답이 왔다 → **에이전트 로직·KB·Gateway 연결 정상**

> **웹 inspector 화면은 이 워크샵에서 필수가 아닙니다.** 배포 후 Lab 6·7 에서 **CloudWatch GenAI Observability** 로 훨씬 자세한 span 트리를 봅니다. 로컬 inspector 는 개발 중 빠른 확인용 도구일 뿐이라, Code Editor 에서 안 열리면 **그냥 건너뛰어도 됩니다.**
>
> 그래도 웹 화면을 보고 싶다면 — `agentcore dev` (플래그 없이) 로 띄운 뒤 `xdg-open` 오류를 무시하고, Code Editor 좌측 **PORTS** 탭 → **Forward a Port** 에 출력된 포트(예: `8081`)를 넣으면 브라우저에서 열립니다. 시연용이 아니면 시간을 쓰지 않는 편이 낫습니다.

> **로컬 노트북**에서 작업한다면 `agentcore dev` 만으로 브라우저가 자동으로 열립니다 (macOS `open` / Windows `start`).
>
> 포트가 이미 쓰이면 `agentcore dev --logs -p 3000` 으로 바꿉니다.
...
## 4단계 전에 — `CDK synth failed` 가 나면

`agentcore deploy` 가 이 메시지로 멈추는 경우가 있습니다:

```
✓ Build CDK project
⠋ Synthesize CloudFormation...
CDK synth failed: node dist/bin/cdk.js: Subprocess exited with error 1
Log: agentcore/.cli/logs/deploy/deploy-20260825-022458.log
```

> **먼저 알아둘 것 — `NodeVersionSupportWarning` 은 원인이 아닙니다.**
>
> ```
...
> ```
>
> 이건 **2027년 이후 SDK 에 대한 사전 안내**일 뿐입니다. 실제 요구사항을 확인하면
> `aws-cdk` 는 `node >= 18`, `aws-cdk-lib` 는 `node >= 20` 이라 **Node 20.19.6 으로 충분합니다**
> (npm registry 로 확인). 경고는 무시하세요.

### ① 실제 오류를 먼저 확인하세요

CLI 가 출력하는 한 줄로는 원인을 알 수 없습니다. **CDK 를 직접 실행하면 상세 오류가 나옵니다**:
...
여기서 나오는 메시지가 진짜 원인입니다. 아래 표에서 해당 항목을 찾으세요.

### ② 원인별 조치

| 상세 오류에 나오는 것 | 원인 | 조치 |
|---|---|---|
| `ENOSPC` / `no space left` | **디스크 부족** — CDK `node_modules` 만 약 300MB, venv 약 800MB | `df -h /home/sagemaker-user` 로 확인. 부족하면 `~/.cache/uv`·`~/.npm` 정리, 또는 Code Editor space 의 EBS 볼륨을 늘리세요 |
| `Cannot find module` / `MODULE_NOT_FOUND` | CDK 의존성 설치가 중간에 끊김 | `cd ~/thewhoo-agentcore-workshop/agentcore/cdk && rm -rf node_modules && npm install` 후 재시도 |
| `Need to perform AWS calls ... but no credentials` / `ExpiredToken` | 자격증명 문제 | `aws sts get-caller-identity` 로 확인. 만료됐으면 **Code Editor 터미널을 닫고 새로 열어** 자격증명을 갱신하세요 |
| `is not authorized to perform` | IAM 권한 부족 | CloudShell 에서 `./scripts/grant-sagemaker-permissions.sh` 재실행 후 **새 터미널** |
| `SSM parameter /cdk-bootstrap/... not found` | CDK bootstrap 미완료 | `npx cdk bootstrap aws://$(aws sts get-caller-identity --query Account --output text)/us-east-1` |
| `agentcore.json` 관련 스키마 오류 | 설정 파일 문제 | `agentcore validate` 로 확인 후 `python3 scripts/set-agentcore-config.py` 재실행 |

### ③ 그래도 안 되면 — 처음부터 다시

CDK 프로젝트만 지우고 재생성하면 대부분 해결됩니다. **`src/` 코드는 건드리지 않습니다**:

```bash
cd ~/thewhoo-agentcore-workshop
rm -rf agentcore                      # CDK 프로젝트만 제거
...
> **진행자용** — 이 오류는 원인이 여러 가지라 한 가지 처방으로 해결되지 않습니다. 참가자가 막히면 **①의 `node dist/bin/cdk.js synth` 를 먼저 실행시켜** 상세 메시지를 확보한 뒤 표에서 찾는 순서로 진행하세요. 시간이 없으면 ③(재생성)이 가장 확실합니다.
