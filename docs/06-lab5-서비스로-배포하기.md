...
### 웹 inspector 를 보려면 — Day 1 Streamlit 과 같은 방식

`--logs` 없이 실행하면 웹 UI(포트 **8081**)가 뜹니다. `xdg-open ENOENT` 오류가 나와도 **서버는 살아 있으니 `Ctrl+C` 하지 마세요.**

> ⚠️ **`http://localhost:8081` 을 노트북 브라우저에 넣으면 안 됩니다.** 그 `localhost` 는 Code Editor 컨테이너 안입니다. **[Day 1 의 Streamlit](streamlit-on-code-editor.md) 과 똑같이 포트 포워딩**이 필요합니다:
>
> 1. **PORTS** 탭 (없으면 `View → Ports`)
> 2. **Forward a Port** → `8081` → Enter
> 3. **지구본 아이콘 (Open in Browser)** 클릭
>
> Day 1 에서 `8501` 로 Streamlit 을 보셨다면 **포트 번호만 다릅니다.**
...
ACC=$(aws sts get-caller-identity --query Account --output text)
cd ~/thewhoo-agentcore-workshop/agentcore/cdk
npx cdk bootstrap aws://$ACC/us-east-1
