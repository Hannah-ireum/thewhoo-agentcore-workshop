...
>
> 🚨 **`cdk bootstrap` 은 반드시 직접, `deploy` 보다 먼저 실행하세요.** `agentcore deploy -y` 의 자동 bootstrap 에 맡기면 **새 계정에서 실패합니다** (실측). **[2.5단계](#25단계-cdk-bootstrap-계정당-1회)** 에서 다룹니다 — `agentcore create` 가 CDK 프로젝트를 만든 뒤에야 실행할 수 있으므로 2단계 다음입니다.

## 전체 흐름 — 이 순서대로만 하면 됩니다

| 단계 | 하는 일 | 핵심 명령 |
|---|---|---|
| **1** | 엔트리포인트 이해 (읽기만) | — |
| **2** | CLI 설치 → 프로젝트 생성 → 우리 코드 가리키게 설정 | `agentcore create` → `set-agentcore-config.py` |
| **2.5** | CDK bootstrap (계정당 1회) | **리포 루트에서** `node agentcore/cdk/node_modules/aws-cdk/bin/cdk bootstrap` |
| **🚦** | **관문 — 여기서 막히면 3·4단계가 전부 실패합니다** | `./scripts/check-agentcore-config.sh` |
| **3** | (선택) 로컬 실행 | `agentcore dev --logs` |
| **4** | **배포** | `agentcore deploy -y` |
| **5** | 호출 | `agentcore invoke "..."` |
| 6·7 | Streamlit UI · 캐싱 관찰 | — |

> **2.5단계를 2단계보다 먼저 할 수 없습니다** — `agentcore/cdk/` 는 2단계의 `agentcore create` 가 만드는 디렉터리입니다.
>
> **🚦 관문을 건너뛰지 마세요.** 실환경에서 확인된 Lab 5 사고의 대부분이 여기서 걸러집니다.
...
새 CLI 는 **프로젝트 전체를 하나의 JSON 으로 선언**합니다. Runtime·Memory·KB·Gateway·Evaluator 를 같은 파일에서 관리하고, 배포 시 **CDK 가 CloudFormation 으로 변환**해 적용합니다.

즉 "배포 스크립트" 가 아니라 **IaC** 입니다. 그래서 `remove` 후 `deploy` 하면 **정확히 되돌아갑니다** — 손으로 만든 리소스는 이게 안 됩니다.
...
agentcore --version          # 0.27 이상이면 정상
```

> `EACCES` 또는 `command not found` → [트러블슈팅](lab5-트러블슈팅.md#증상별-빠른-조회). **`sudo npm install -g` 는 쓰지 마세요** — root 소유 파일이 생겨 이후 재설치가 막힙니다.
...
mv ThewhooChat/agentcore ./       # 설정 + CDK 프로젝트만 리포 루트로
rm -rf ThewhooChat                # 쓰지 않는 스캐폴딩 코드 제거

ls agentcore/                     # agentcore.json · aws-targets.json · cdk/ 가 보이면 정상
```

> **왜 옮기나** — `create` 는 `ThewhooChat/` 하위에 새 에이전트 코드를 만듭니다. 우리는 **Lab 1-4 에서 만든 `src/` 를 배포**하므로 스캐폴딩(`app/`)은 버리고 설정 디렉터리(`agentcore/`)만 가져옵니다.
>
> **`--memory none` 인 이유** — Memory 는 Lab 2 에서 이미 만들었습니다. 여기서 또 만들면 두 개가 됩니다. 기존 것은 `envVars` 로 ID 를 넘겨 씁니다. (새 프로젝트라면 `--memory longAndShortTerm` 한 줄로 4-strategy 가 자동 선언됩니다.)

▶ **실행 ③ — 우리 코드를 가리키게 설정** (손으로 편집하지 마세요)
...
> ⚠️ **`print-env.sh` 를 먼저 하지 않으면** 스크립트가 `[ERROR] 환경변수 누락` 으로 멈춥니다. 그 상태에서 JSON 을 손으로 고치면 `codeLocation` 이 `app/<이름>/` 로 남아 **dev·deploy 가 모두 실패**합니다 (아래 🚦 관문이 잡아냅니다).

### 스크립트가 채운 값 — 각 필드가 무엇인가
...
```json
{
  ...
  "codeLocation": "src/",
  "runtimeVersion": "PYTHON_3_12",
  "envVars": [ { "name": "KB_ID", "value": "..." } ],
  "executionRoleArn": "arn:aws:iam::<account>:role/thewhoo-agent-role-w001"
}
```

| 필드 | 역할 | 주의 |
|---|---|---|
| `codeLocation` | zip 에 담길 디렉터리 | 이 아래 전체가 패키징됩니다 |
| `entrypoint` | `@app.entrypoint` 가 있는 파일 | `codeLocation` **기준 상대경로** |
| `runtimeVersion` | Python 버전 | 기본값이 `PYTHON_3_14` — 의존성 호환을 위해 **3.12 로 낮춥니다** |
| `envVars` | 컨테이너 환경변수 | **객체가 아니라 배열**입니다 (`[{"name":…,"value":…}]`) |
| `executionRoleArn` | Runtime 이 쓸 IAM role | **비우면 안 됩니다** — 아래 |

> **`executionRoleArn` 을 비우면 첫 invoke 가 500 입니다.** CDK 가 role 을 자동 생성하지만 **모델 호출 권한만** 줍니다 — Memory·KB·Gateway 권한이 없습니다. `--memory none` 으로 만들었으니 CDK 가 Memory 권한을 줄 이유가 없고, 우리는 Lab 2 에서 **직접 만든** Memory 를 쓰기 때문에 생기는 간극입니다.
>
> 위 스크립트가 워크샵 role(`thewhoo-agent-role-<pid>`)을 자동으로 넣습니다 — 별도 명령이 없습니다. 계정 ID 는 STS 로 조회합니다.
>
> 증상과 CloudWatch 원인 로그는 [트러블슈팅](lab5-트러블슈팅.md#증상별-빠른-조회) 에 있습니다.

> **`src/pyproject.toml` 이 필요합니다.** 새 CLI 는 `requirements.txt` 를 읽지 않습니다 — 없으면 `CDK synthesis failed: Required project file not found` 가 납니다. 저장소에 이미 포함돼 있습니다. (`requirements.txt` 는 로컬 개발용, `pyproject.toml` 은 **배포 패키징용**입니다.)

### 실제 서비스에 적용할 때 조정할 것

- **`executionRoleArn`** — 워크샵 role 은 넓습니다(`bedrock-agentcore:*`). 프로덕션은 리소스 ARN 별로 제한하세요.
- **`build: Container`** — 시스템 패키지가 필요하거나 250MB 를 넘으면 전환
- **`networkMode: VPC`** — 사내 API 를 호출해야 하면
- **여러 환경** — `aws-targets.json` 에 dev/prod 를 나누고 `--target` 으로 선택

## 2.5단계. CDK bootstrap (계정당 1회)

AgentCore CLI 는 배포를 **CDK 로** 수행하므로, 리전에 bootstrap 스택(`CDKToolkit`)이 먼저 있어야 합니다. **`deploy` 의 자동 bootstrap 에 맡기지 말고 직접 실행하세요** — 새 계정에서 실패하고, 그 실패가 스택을 못 쓰는 상태로 남겨 이후 배포까지 막습니다.

> **왜 여기인가** — `agentcore/cdk/` 는 **2단계의 `agentcore create` 가 만드는 디렉터리**입니다. 그전에는 존재하지 않으므로 이 단계를 2단계보다 앞에서 실행할 수 없습니다.
...
> ⚠️ **`agentcore/cdk` 안에서 실행하지 마세요.** 그 디렉터리의 `cdk.json` 이 `"app": "node dist/bin/cdk.js"` 를 지정하는데, `dist/` 는 **TypeScript 빌드 산출물이라 배포 전에는 존재하지 않습니다.** CDK 가 명령 실행 전에 이 `app` 을 평가하려다 실패합니다 (실측):
>
> ```
> Error: Cannot find module '.../agentcore/cdk/dist/bin/cdk.js'
> node dist/bin/cdk.js: Subprocess exited with error 1
> ```
>
> **bootstrap 은 우리 앱을 합성할 필요가 없습니다** — AWS 가 제공하는 고정 템플릿을 배포하는 것뿐입니다. 그래서 **리포 루트에서** 실행하면 `cdk.json` 을 읽지 않아 이 문제가 없습니다.

```bash
ACC=$(aws sts get-caller-identity --query Account --output text); echo "계정: $ACC"
```

계정 번호가 찍히는지 확인한 뒤 (2~3분 걸립니다):

```bash
node agentcore/cdk/node_modules/aws-cdk/bin/cdk bootstrap aws://$ACC/us-east-1
```

> **경로 두 부분을 나눠 보세요** — `agentcore/cdk/node_modules/aws-cdk/bin/cdk` 는 **CDK 실행 파일의 위치**이고, 실행되는 **현재 디렉터리는 리포 루트**입니다. `npx cdk` 는 Code Editor 에서 출력 없이 조용히 종료되므로 쓰지 마세요 (실측).

`✅ Environment aws://<계정ID>/us-east-1 bootstrapped.` 가 나오면 성공입니다.
IAM role 4개와 S3 버킷·ECR 리포지토리가 만들어지는 과정이 진행률로 표시됩니다.

> `NodeVersionSupportWarning` 과 `Newer version of CDK is available` 는 무시하세요.

**계정당 1회**입니다. 실패하거나 이미 깨진 상태라면 → [배포 트러블슈팅](lab5-트러블슈팅.md#cdk-bootstrap-이-실패할-때)

## 🚦 관문 — **여기를 통과하지 않으면 3·4단계가 전부 실패합니다**

2단계(설정)와 2.5단계(bootstrap)가 제대로 끝났는지 **한 번에** 확인합니다.
...
✓ CDK bootstrap 완료 (CDKToolkit=CREATE_COMPLETE)

✓ 사전점검 통과 — agentcore dev / deploy 를 진행하세요
```

**이 화면이 나와야 다음으로 갑니다.** `✗` 가 있으면 항목마다 조치 방법이 함께 출력됩니다.
...
> | `✗` 항목 | 돌아갈 곳 |
> |---|---|
> | `codeLocation` · `entrypoint` · `envVars` · `executionRoleArn` | **2단계** — `eval "$(./scripts/print-env.sh w001)"` 후 `python3 scripts/set-agentcore-config.py` |
> | `CDK bootstrap` | **2.5단계** — 리포 루트에서 bootstrap |
> | `uv` · `node` · `agentcore` | 맨 위 전제조건 표 |
>
> 조치 후 `./scripts/check-agentcore-config.sh` 로 다시 확인하세요.
...
→ **[Lab 5 배포 트러블슈팅](lab5-트러블슈팅.md)** (`Failed to create venv`, 웹 UI 를 브라우저로 보는 방법 등)

## 4단계 전에 — 배포가 실패할 때

배포가 실패하면 **먼저 관문을 실행하세요.** 대부분 여기서 원인이 나옵니다.

```bash
./scripts/check-agentcore-config.sh
```

관문을 통과했는데도 실패하면 → **[Lab 5 배포 트러블슈팅](lab5-트러블슈팅.md)**
...
실제 서비스는 CLI 가 아니라 **코드로** 호출합니다. 아래는 **터미널에 그대로 붙여넣으면 실행되는** 형태입니다 — ARN 을 `agentcore status` 에서 자동으로 뽑아 넣습니다.

```bash
cd ~/thewhoo-agentcore-workshop
source .venv/bin/activate
ARN=$(agentcore status 2>/dev/null | grep -o 'arn:aws:bedrock-agentcore:[^)]*runtime/[A-Za-z0-9_-]*' | head -1)
echo "ARN: $ARN"
```
...
> `python3 - "$ARN" <<'PY' ... PY` 는 **heredoc** 입니다 — 파일을 만들지 않고 Python 코드를 바로 실행합니다. 실제 서비스에서는 이 코드를 `.py` 파일이나 애플리케이션 안에 두면 됩니다.
...
3. span 트리에서 **`chat us.anthropic.claude-sonnet-4-6`** span 클릭 — 이것이 **Orchestrator 의 모델 호출** 입니다
...
> ⚠️ **`orchestrator` 나 `LLM` 이라는 이름의 span 은 없습니다.** 모델 호출 span 의 이름은 **`chat <모델ID>`** 입니다 (`strands/telemetry/tracer.py` 실측). 실제 트리는 이렇게 보입니다:
>
> ```
> POST /invocations                            ← root
>   invoke_agent Strands Agents
>     execute_event_loop_cycle
>     chat us.anthropic.claude-sonnet-4-6      ← ⭐ Orchestrator (여기를 클릭)
>     chat us.anthropic.claude-haiku-4-5…      ← 서브에이전트
>     execute_tool <도구명>
> ```
>
> **모델 ID 로 구분하세요** — Orchestrator 는 Sonnet, 서브에이전트는 Haiku 입니다. `chat` span 이 여러 개 보이는 것이 정상이고, 캐시 토큰은 이 `chat` span 에 호출별로 박힙니다. 전체 span 구성은 [Lab 6](07-lab6-운영-상태-들여다보기.md) 에서 자세히 다룹니다.
...
증상별 조치는 **[Lab 5 배포 트러블슈팅](lab5-트러블슈팅.md#증상별-빠른-조회)** 에 모아 두었습니다.
