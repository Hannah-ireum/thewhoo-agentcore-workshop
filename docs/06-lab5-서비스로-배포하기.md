...
>
> **CLI 안내**: 이 워크샵은 Python `bedrock-agentcore-starter-toolkit` 의 `agentcore` CLI 를 사용합니다 (`pip install bedrock-agentcore-starter-toolkit`). 2026 년 들어 AWS 가 새 CLI 인 `@aws/agentcore` (npm) 를 권장하기 시작했지만, Python 기반 starter-toolkit 도 계속 동작합니다. 실행 시 보이는 deprecation banner 는 `AGENTCORE_SUPPRESS_RECOMMENDATION=1` 환경변수로 끌 수 있습니다.
