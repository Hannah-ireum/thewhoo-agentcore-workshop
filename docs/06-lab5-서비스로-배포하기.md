...
### 🚨 Code Editor 에서는 `--logs` 를 쓰세요 (터미널 2개 사용)

Code Editor 는 **원격 리눅스 컨테이너**입니다. 브라우저를 열 프로그램(`xdg-open`)이 없어서 그냥 `agentcore dev` 를 돌리면 이렇게 됩니다 (실측):
...
`--no-browser` 도 Code Editor 에서는 안 됩니다 (실측):

```
Error: This command requires an interactive terminal.
```

> 이 플래그는 TUI(터미널 대화형 화면)를 띄우려 해서 진짜 TTY 가 필요합니다. Code Editor 터미널에서 열리지 않는 경우가 있습니다.

**Code Editor 에서 확실히 되는 방법은 `--logs` 입니다.** 서버 로그를 화면에 흘리며 실행하고, **호출은 두 번째 터미널에서** 합니다.
...
`Application startup complete.` 가 보이면 준비된 것입니다. **이 터미널은 그대로 두세요** (여기서 요청 로그가 실시간으로 흐릅니다).

▶ **실행 — 터미널 ②: 호출하기**

Code Editor 상단 메뉴 **Terminal → New Terminal** 로 두 번째 터미널을 엽니다.
...
답변이 터미널에 바로 출력됩니다 (실측):

```
2 더하기 3은 **5**입니다.      ← 예시 프로젝트의 응답 형태
```

▶ **대안 — `curl` 로 직접 호출**

`agentcore dev "..."` 가 안 되면 HTTP 로 직접 부를 수 있습니다. 이쪽이 **가장 확실**합니다:

```bash
curl -N -X POST http://localhost:8080/invocations \
  -H 'Content-Type: application/json' \
  -d '{"prompt":"천기단 화현 크림 성분 알려줘"}'
```

응답은 **SSE 스트림**으로 옵니다 (실측):

```
data: {"event": {"messageStart": {"role": "assistant"}}}
data: {"event": {"contentBlockDelta": {"delta": {"text": "천기단..."}}}}
...
```

> `-N` 은 버퍼링을 끄는 옵션입니다. 스트림이 아니라 최종 텍스트만 보려면 `agentcore dev "..."` 쪽이 읽기 편합니다.
