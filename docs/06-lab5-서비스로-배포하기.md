...
>
> 이전 버전 워크샵은 Python `bedrock-agentcore-starter-toolkit` 을 썼습니다. 그 CLI 를 실행하면 *"The Starter Toolkit CLI is no longer supported ... New Bedrock AgentCore features are only accessible in the AgentCore CLI"* 배너가 출력됩니다 (도구 자체가 내보내는 안내이며, devguide 의 release notes 에는 별도 지원 종료 공지가 없습니다). 공식 [Get started](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/runtime-getting-started.html) 문서도 AgentCore CLI 를 안내합니다. 두 CLI 의 차이와 기존 프로젝트 이전 방법은 [새 CLI 이전 가이드](새-cli-이전-가이드.md) 를 참고하세요.
>
