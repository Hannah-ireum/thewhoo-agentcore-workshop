...
> `No such file or directory` 가 나면 **2단계를 아직 안 한 것입니다.** 위로 돌아가 `agentcore create` 부터 하세요.
...
> ⚠️ **경로를 그대로 쓰세요.** 두 가지 대안은 실환경에서 각각 실패합니다 (실측):
>
> | 쓰지 말 것 | 증상 | 이유 |
> |---|---|---|
> | `npx cdk …` | **출력 없이 즉시 종료** (성공처럼 보이지만 스택이 안 생김) | — |
> | `./node_modules/.bin/cdk …` | `Cannot find module '.../cdk/dist/bin/cdk.js'` | CDK 프로젝트 자신도 `bin: {"cdk": "dist/bin/cdk.js"}` 를 선언해서, `.bin/cdk` 가 **아직 빌드되지 않은 `dist/`** 를 가리키는 경우가 있습니다 |
>
> `node node_modules/aws-cdk/bin/cdk` 는 **aws-cdk 패키지를 직접 지정**하므로 이 혼동이 없습니다.
