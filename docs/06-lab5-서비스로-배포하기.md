...
> ### ⏭️ 시간이 없으면 이 단계를 건너뛰고 [4단계 배포](#4단계-agentcore-runtime-에-배포)로 가세요
>
> `agentcore dev` 는 **배포 전 예비 확인**입니다. Lab 5 의 목표는 4단계 배포이고, 여기서 막혀도 배포·호출·관측(Lab 6·7)에는 전혀 영향이 없습니다. Code Editor 에서 이 단계는 환경 제약이 많아 **참가자가 가장 많이 시간을 잃는 지점**입니다.

### 개념 먼저

`agentcore dev` 는 배포 없이 로컬에서 Runtime 환경을 흉내 냅니다. 컨테이너 빌드·업로드를 건너뛰므로 코드 수정 → 확인 루프가 빠릅니다. 내부적으로 `uv venv` + `uv sync` 로 **`codeLocation`(=`src/`) 안에 별도 `.venv` 를 새로 만듭니다** — 루트의 워크샵 venv 와는 다른 것입니다.

### Code Editor 에서 자주 나오는 두 오류
...
원격 컨테이너라 브라우저를 열 프로그램이 없습니다. **서버 자체는 떠 있습니다** (`Ctrl+C` 로 끄면 죽습니다).

> ⚠️ **`http://localhost:8081` 을 노트북 브라우저에 넣으면 안 됩니다.** 그 `localhost` 는 Code Editor 컨테이너 안입니다. **Day 1 의 Streamlit 과 똑같이 포트 포워딩**이 필요합니다 — [Streamlit on Code Editor 가이드](streamlit-on-code-editor.md) 와 동일한 절차입니다:
>
> 1. Code Editor 하단/좌측 **PORTS** 탭 (없으면 `View → Ports`)
> 2. **Forward a Port** → 터미널에 나온 포트(`8081`) 입력 → Enter
> 3. 그 행의 **지구본 아이콘(Open in Browser)** 클릭
>
> Day 1 에서 `8501` 로 Streamlit 을 보셨다면 **포트 번호만 바뀐 것**입니다.
...
CLI 가 `src/` 안에 새 venv 를 만들려다 실패한 것입니다. `unknown error` 는 CLI 가 `uv` 의 stderr 를 못 받아 그대로 출력한 것이라 **메시지만으로는 원인을 알 수 없습니다.**

▶ **원인을 직접 확인**

```bash
cd ~/thewhoo-agentcore-workshop/src
uv venv          # 여기서 나오는 메시지가 진짜 원인
uv sync          # venv 가 됐으면 다음 단계도 확인
```

| `uv venv` 가 내는 메시지 | 원인 | 조치 |
|---|---|---|
| `No interpreter found for Python 3.1x` | `pyproject.toml` 의 `requires-python` 과 설치된 Python 이 불일치 | 저장소 최신본은 `>=3.11` 로 맞춰져 있습니다 — `git pull` 후 재시도 |
| `No space left on device` / `ENOSPC` | 디스크 부족 (루트 venv 800MB + CDK 300MB) | `df -h /home/sagemaker-user` 확인 → `rm -rf ~/.cache/uv ~/.npm` |
| `Permission denied` | `src/` 쓰기 권한 | `ls -ld src` 확인 |
| `uv: command not found` | `uv` 미설치 | `curl -LsSf https://astral.sh/uv/install.sh \| sh` 후 새 터미널 |

▶ **정리하고 다시**

```bash
rm -rf ~/thewhoo-agentcore-workshop/src/.venv    # 반쯤 만들어진 venv 제거
cd ~/thewhoo-agentcore-workshop
agentcore dev --logs
```

> **여기서도 안 되면 그냥 4단계로 가세요.** 배포된 Runtime 은 `src/.venv` 를 쓰지 않습니다 (컨테이너가 `pyproject.toml` 로 의존성을 새로 설치합니다). 즉 **이 오류는 배포와 무관합니다.**
...
확인할 것은 **두 개뿐**입니다: ① 서버 기동 문구 ② 답변 수신.

> `--no-browser` 는 쓰지 마세요 — `Error: This command requires an interactive terminal` 로 실패합니다 (실측).
>
> 포트 구분: `agentcore dev` = 웹 UI **8081** / `agentcore dev --logs` = API **8080**.
