...
### 🚨 Code Editor 에서는 `--no-browser` 를 쓰세요

SageMaker Code Editor 는 **원격 리눅스 컨테이너**입니다. 브라우저를 열 프로그램(`xdg-open`)이 없어서 그냥 `agentcore dev` 를 돌리면 이 오류가 납니다 (실환경 실측):
...
> **중요 — 서버는 정상적으로 떴습니다.** `Chat UI: http://...` 가 출력된 뒤에 브라우저만 못 연 것입니다. CLI 는 서버를 띄운 다음 `xdg-open` 을 호출하는데, 그 호출이 분리 실행(`detached`)이라 실패해도 서버는 살아 있습니다. 참가자가 "에러가 났다" 며 `Ctrl+C` 로 끄는 게 가장 흔한 실수입니다.

▶ **실행 — 방법 A: 터미널 TUI (권장, 가장 간단)**
...
브라우저 대신 터미널 안에서 채팅합니다. 별도 설정이 필요 없습니다.

▶ **실행 — 방법 B: 웹 UI 를 Code Editor 포트 프록시로 보기**

웹 inspector 화면을 꼭 보고 싶다면 (고객 시연 등):

```bash
agentcore dev            # 그대로 실행 — xdg-open 오류는 무시하고 Ctrl+C 하지 마세요
```

출력된 포트(예: `8081`)를 Code Editor 가 프록시해 줍니다:

1. 좌측 사이드바 **PORTS** 탭 (안 보이면 `Ctrl+Shift+P` → `Ports: Focus on Ports View`)
2. **Forward a Port** → 출력된 포트 번호 입력
3. 생성된 링크를 클릭 — 또는 아래 형태로 직접 접속

```
https://<studio-id>.studio.<region>.sagemaker.aws/codeeditor/default/ports/8081/
```

> `<studio-id>` 는 Code Editor 브라우저 주소창의 첫 부분입니다. 이 URL 은 **SageMaker 세션 인증이 필요**하므로 같은 브라우저에서만 열립니다 (외부에서 접근하면 403).
>
> 포트 번호는 실행할 때마다 다를 수 있습니다 — 터미널에 출력된 `Chat UI:` 줄의 포트를 쓰세요. `agentcore dev -p 3000` 으로 고정할 수도 있습니다.
...
>
> **로컬 노트북**에서 작업한다면 `agentcore dev` 만으로 브라우저가 자동으로 열립니다 (macOS `open` / Windows `start` 를 씁니다).
...
| `agentcore dev` 가 `Error: spawn xdg-open ENOENT` | **서버는 정상 실행됐고 브라우저만 못 연 것입니다** (Code Editor 는 원격 컨테이너라 브라우저가 없음). `Ctrl+C` 로 끄지 마세요. `agentcore dev --no-browser` 로 터미널 TUI 를 쓰거나, PORTS 탭으로 포트를 포워딩해 웹 UI 를 보세요 (위 3단계 참고) |
