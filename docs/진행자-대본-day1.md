...
> 도구 1-5개면 OFF 가 단순합니다. 20개 이상이거나 동적으로 추가되면 ON 을 고려하세요. **생성 시점에 정하는 게 원칙**입니다 — API 상 `update-gateway` 로도 바꿀 수 있지만 필수 필드와 기존 설정을 모두 다시 넘겨야 해서 실습 중엔 재생성이 안전합니다.
>
> 그리고 **quota 를 짚어주세요** — search 기반 tool-call 은 **분당 25회**입니다. 참가자 여러 명이 같은 계정에서 이 절을 반복하면 throttling 이 날 수 있습니다.
