...
> **용어 정리** — "골든셋(golden set)" 은 ML 평가에서 널리 쓰이는 일반 용어이고, **AWS 공식 문서는 이것을 `dataset`(평가 데이터셋)이라고 부릅니다.** 공식 문서를 찾을 때는 아래 대응을 참고하세요:
>
> | 이 워크샵 | AWS 공식 용어 |
> |---|---|
> | 골든셋 | **dataset** / evaluation dataset |
> | 시나리오 1건 | **scenario** |
> | 정답 (`assertions` · `expected_trajectory`) | **ground truth** |
> | 회귀 게이트 | **regression testing** |
>
> 공식 문서 원문도 용도를 같게 설명합니다 — *"useful for **regression testing**, benchmark datasets, CI/CD pipelines, baseline measurement"* ([Dataset evaluation](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/dataset-evaluations.html)). 이후 이 문서에서는 읽기 쉬운 "골든셋" 을 쓰고, 필드·API 이름은 공식 표기를 그대로 씁니다.

```
매 release →  골든셋 전체 invoke  →  evaluator 채점  →  미달이 하나라도 있으면 회귀 → release 차단
```

사람이 매번 모든 답변을 검토할 필요가 없고, **"지난 release 엔 통과했는데 이번엔 실패"** 한 시나리오만 빠르게 추적할 수 있습니다.

이 워크샵 골든셋은 공식 [Dataset schema](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/dataset-evaluations-schema.html) 그대로 작성했습니다 (`FileDatasetProvider` 로 20개 전부 `PredefinedScenario` 로 인식되는 것을 확인).

### 이 워크샵의 골든셋 — 20 시나리오

`docs/eval/golden-set.json` 에 있습니다. `scenario_id` prefix 가 카테고리입니다.

공식 스키마의 **Predefined scenario** 형식을 그대로 씁니다 (전부 단일 턴):

| 필드 | 이 골든셋 | 공식 필수 |
|---|---|---|
| `scenario_id` | 20/20 | **Yes** |
| `turns` | 20/20 | **Yes** |
| `assertions` | 20/20 | No |
| `expected_trajectory` | **19/20** — `SAFE_Q20` 만 도구 호출이 없음 | No |
| `metadata` · `expected_response` | 미사용 | No |

| `scenario_id` | 카테고리 | 검증 포인트 |
|---|---|---|
| `INFO_Q01`~`Q06` | 정보성 | 실제 상품의 성분·사용법·주의사항 |
| `REC_Q07`~`Q11` | 추천 | 피부 타입별 / 카테고리별 |
| `STOCK_Q12`~`Q14` | 재고 | 가용 여부 / 배송 (`Q14` 는 품절 — 없는 배송일을 만들어내지 않는지) |
| `PROMO_Q15`~`Q16` | 프로모션 | 진행중 혜택 |
| `MIX_Q17`~`Q19` | 복합 | 추천 + 프로모션 / 재고 / 가격대 |
...
| 필드 | 채점하는 것 | 채점 방식 |
|---|---|---|
| `assertions` | **답변 내용** — 자연어 조건 | judge LLM (`GoalSuccessRate`) |
| `expected_trajectory` | **도구 선택** — 호출 순서 | 프로그램 (`Trajectory*Match`) · **비용 0** |

### assertion 작성 원칙 4가지

공식 문서는 형식을 규정하지 않고 **자유 형식 자연어**라고 명시합니다 — *"assertions are free-form natural language… such as response tone, factual accuracy, safety compliance, or business logic"* ([Ground truth evaluations](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/ground-truth-evaluations.html)). 실무에서 그대로 쓰실 수 있는 원칙입니다:

1. **키워드 일치가 아니라 의미로 쓴다** — "장뇌삼이 포함되어야 한다" 가 아니라 "**최소 두 가지를** 언급해야 한다". 답변 문구는 매번 달라지므로 judge 가 판단할 여지를 남깁니다.
2. **한 assertion = 한 검증 항목** — 여러 조건을 한 문장에 몰면 부분 실패를 구분할 수 없습니다.
3. **실제 데이터에서 근거를 가져온다** — 지어낸 정답으로 채점하면 회귀 테스트가 아니라 **오답 고정**입니다.
4. **도구 검증은 `expected_trajectory` 로 넘긴다** — assertion 으로 쓰면 judge 가 토큰을 쓰지만 `Trajectory*Match` 는 **비용 0** 입니다.

> 정확한 키워드 일치가 꼭 필요하면 [code-based evaluator](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/code-based-evaluators.html) 를 추가하는 것이 공식 권장입니다.

### ⚠️ 정답은 반드시 `evaluationReferenceInputs` 로 넘겨야 합니다

**골든셋 파일에 적어 두기만 하면 evaluator 가 알아서 쓰는 게 아닙니다.** `Evaluate` 호출 시 이 형태로 전달해야 합니다:

```python
evaluationReferenceInputs=[{
    "context": {"spanContext": {"sessionId": SESSION_ID}},   # 필수
...
빼먹으면 evaluator 가 **ground-truth-free 모드**로 돌아갑니다 — **골든셋과 무관한 기준으로 채점하는데도 PASS 가 나옵니다.** `scripts/run-golden-eval.py` 의 `build_reference_inputs()` 가 이 변환을 담당하므로, 워크샵 스크립트를 쓰면 신경 쓸 필요가 없습니다.

> ground truth 를 넘기면 `GoalSuccessRate` 의 verdict 어휘가 `Yes`/`No` → `SUCCESS`/`FAILURE` 로 바뀝니다 (prompt template 이 달라짐). 스크립트의 `DEFAULT_GATE` 는 두 어휘를 모두 허용합니다.
>
> 필드별 적용 범위·`expected_response` 매핑 규칙 등 세부는 → [부록](day2-부록-근거자료.md)

### trajectory evaluator 3종 — 어느 것을 쓰나

셋 다 **programmatic scoring** 이라 LLM 을 호출하지 않습니다 (*"no LLM calls, so token usage is zero"*).

기대가 `[qa_tool, recommend_tool]` 인데 실제가 `[qa_tool, x, recommend_tool]` 이라면:

| evaluator | 판정 | 결과 |
|---|---|---|
| `TrajectoryExactOrderMatch` | 정확히 일치, 추가 도구 불허 | ❌ FAIL |
| **`TrajectoryInOrderMatch`** (이 워크샵) | 순서대로 등장, 사이에 추가 허용 | ✅ PASS |
| `TrajectoryAnyOrderMatch` | 모두 존재, 순서 무관 | ✅ PASS |

> **`InOrderMatch` 를 쓰는 이유** — 에이전트가 재시도나 추가 조회를 하는 건 정상이라 `Exact` 는 너무 엄격하고, 순서가 의미를 갖는 경우(검색 → 추천)가 있어 `AnyOrder` 는 너무 느슨합니다.
>
> **비용 0 을 활용하세요** — 골든셋을 키우면 judge evaluator 는 토큰 quota 에 걸리지만 trajectory 계열은 무료입니다. 도구 회귀만 넓게 잡으려면 trajectory 중심 구성이 경제적입니다.

> ⚠️ **Preview 안내** — **Dataset evaluation** 기능(`FileDatasetProvider` · `OnDemandEvaluationDatasetRunner`)은 현재 **public preview** 입니다. API·필드명이 바뀔 수 있습니다. `Builtin.*` 를 직접 호출하는 `evaluate` API 자체는 안정적입니다.
...
> **Online 이 골든셋에 쓸 수 없는 이유** — 위 표의 ground truth 행이 답입니다. 공식 문서가 이유까지 명시합니다:
>
> *"…cannot be used in online evaluation configurations, because online evaluations monitor live production traffic where ground truth values are not available."*
> — [Ground truth evaluations](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/ground-truth-evaluations.html)
>
> API 스키마로도 교차 확인했습니다 — 정답을 넘길 필드가 아예 없습니다 ([부록](day2-부록-근거자료.md#online-평가에는-ground-truth-를-넘길-통로가-없다)).
>
...
> ⚠️ **`Builtin.ToolSelectionAccuracy` 는 여기에 넣지 마세요.** 이 evaluator 는 **tool span 을 대상으로 채점**하는데, 실환경에서 아래 오류로 실패하는 경우가 있습니다 (실측):
>
> ```
> Error: The evaluation target contains spanIds that do not exist in the provided data.
> Missing spanIds: {'fa0b48931de2eb53', ...}
> ```
>
> CLI 가 채점 대상으로 지목한 tool span 이 평가 요청에 담기지 않아 생기는 불일치입니다. **5분 더 기다려도 같은 spanId 가 그대로 나오므로 인덱싱 지연이 아닙니다.** 도구 선택 정확도를 보려면 4단계의 `expected_trajectory` 기반 **`Builtin.Trajectory*Match`** 계열을 쓰세요 — 프로그램적으로 채점하므로 이 문제가 없고 **비용도 0** 입니다.
>
> | 필요한 span | evaluator |
> |---|---|
> | 없음 (대화만) | `Builtin.Helpfulness` · `Builtin.GoalSuccessRate` · `Builtin.Correctness` |
> | **tool span** | `Builtin.ToolSelectionAccuracy` ⚠️ |
> | `expected_trajectory` | `Builtin.Trajectory*Match` ✅ 권장 |
...
```
aggregateScore : 0.932        ← 전체 평균 (이 evaluator 기준)
sessionScores[] : turn 별 결과
  ├ traceId     : 이 turn 의 trace ID  ← ⭐ 답변 본문을 찾는 열쇠
  ├ value       : 0~1 정규화 점수
  ├ label       : 등급명 (Above And Beyond / Very Helpful ...)
  └ explanation : judge 가 그 점수를 준 이유
tokenUsage      : judge LLM 이 쓴 토큰 (평가 비용)
```

> `sessionId` 는 5턴이 모두 같고 **`traceId` 가 turn 마다 다릅니다.** 즉 한 세션의 각 턴이 개별 채점됩니다.

### ⭐ 에이전트가 실제로 뭐라고 답했는지 보려면

**평가 결과 JSON 에는 답변 본문이 들어 있지 않습니다.** judge 의 채점 결과만 담깁니다. 답변을 보려면 `traceId` 로 trace 를 여세요 — 두 가지 방법이 있습니다.

▶ **방법 1 — CLI 로 내려받기** (빠름)

```bash
agentcore traces get 6a8d858e00c49ae775cf79c90b6bd3e2 --output /tmp/trace.json
```

> `traceId` 는 위 JSON 의 `sessionScores[].traceId` 에서 복사하세요. **점수가 낮은 turn 의 traceId** 를 고르면 바로 원인을 볼 수 있습니다.

내려받은 JSON 에서 입력·출력만 뽑아 보기:

```bash
python3 - <<'PY'
import json
spans = json.load(open('/tmp/trace.json'))
spans = spans if isinstance(spans, list) else spans.get('spans', [])
for s in spans:
    a = s.get('attributes', {})
    for k, v in a.items():
        if any(t in k for t in ('input.value', 'output.value', 'gen_ai.prompt', 'gen_ai.completion')):
            print(f"── {s.get('name')} · {k}\n{str(v)[:1500]}\n")
PY
```

▶ **방법 2 — 콘솔에서 보기** (고객 시연에 적합)

CloudWatch → **GenAI Observability** → Agents → `ThewhooChat` → **Sessions** → 해당 세션 → 그 trace → span 트리에서 **`invoke_agent Strands Agents`** span 클릭 → 우측 패널의 **Input / Output**.

> **시연할 때는 방법 2 를 쓰세요.** "점수 0.83" 이라는 숫자에서 **실제 답변 문장까지** 화면에서 내려가는 흐름이 설득력이 있습니다.

### ⚠️ `explanation` 이 영어로 나오는 이유

**Built-in evaluator 는 영어로 채점합니다.** `Builtin.*` evaluator 의 판정 프롬프트가 영어로 고정돼 있어, 한국어 대화를 평가해도 설명은 영어로 나옵니다. **점수 자체는 한국어 답변을 정확히 이해하고 산출됩니다** — 위 실측 예에서 judge 가 `WHOO-00101` 이 아이크림이 아니라 천기단 화현 크림이라는 **라벨 오류까지 잡아냈습니다** (그래서 1.0 이 아니라 0.83).

**한국어 설명이 필요하면 custom evaluator 를 만드세요.** `--instructions` 에 한국어로 지시하면 그 언어로 채점합니다:

```bash
agentcore add evaluator --name KoHelpfulness \
  --level TRACE --type llm-as-a-judge \
  --rating-scale 1-5-quality \
  --instructions "다음 대화가 사용자의 질문에 얼마나 도움이 되었는지 평가하세요. 반드시 한국어로 이유를 설명하세요.

{context}"

agentcore deploy -y
agentcore run eval --session-id "${SESSION_ID}" --evaluator KoHelpfulness
```

> `{context}` **placeholder 는 반드시 넣어야 합니다** — 평가 대상 대화가 그 자리에 주입됩니다. `--level` 에 맞는 placeholder 를 써야 하며(SESSION/TRACE/TOOL_CALL), 빠지면 evaluator 생성이 거부됩니다.
>
> **고객 보고용이라면 custom evaluator 가 유리합니다.** 다만 built-in 과 **점수 체계가 달라져 과거 결과와 직접 비교할 수 없으니**, 회귀 게이트는 built-in 으로 두고 보고용만 따로 두는 구성을 권합니다.

### 점수가 낮을 때 보는 순서

1. **Explanation 먼저 읽기** — judge LLM 이 "왜" 그 점수를 줬는지 한 단락으로 설명합니다. 대부분 여기서 원인이 드러남.
2. **`traceId` 로 답변 본문 확인** — 위 ⭐ 절의 방법으로 실제 답변을 봅니다.
...
## 6.5단계. 결과를 한 장으로 보기 — HTML 리포트

CLI 출력은 스크롤로 흘러갑니다. 20 시나리오 × evaluator 4종 = **80개 판정**을 한눈에 보려면 리포트를 만드세요. 화면 공유로 설명할 때 특히 유용합니다.

▶ **실행**

```bash
# 1) 평가하면서 결과를 JSON 으로 남긴다
...
```
✓ /tmp/eval.html  (18/20 PASS)

보기:
  macOS       open /tmp/eval.html
  Code Editor 파일 우클릭 → Open Preview
```
...
| 영역 | 내용 |
|---|---|
| 상단 카드 | 시나리오 수 · PASS · FAIL · evaluator 수 |
| 매트릭스 | 행=시나리오, 열=evaluator. 각 칸에 PASS/FAIL 배지 + 점수 요약 |
| FAIL 강조 | 실패한 시나리오 ID 가 **빨간 굵은 글씨** — 스크롤 없이 바로 찾힘 |
| 게이트 기준 | 접이식으로 `DEFAULT_GATE` 원본 표시 |

> **왜 매트릭스인가** — "어느 시나리오가 깨졌나" 만으로는 부족합니다. **어느 evaluator 에서** 깨졌는지가 원인을 가릅니다. `TrajectoryInOrderMatch` 만 FAIL 이면 도구 선택 회귀이고, `Helpfulness` 만 낮으면 답변 품질 문제입니다. 열 단위로 보면 이 구분이 즉시 됩니다.
>
> `--report` 는 CI 산출물로도 씁니다 — GitHub Actions 의 artifact 로 올려 두면 PR 에서 바로 열어볼 수 있습니다 (7단계 참고).
...
| `ToolSelectionAccuracy` 가 `['Yes','Yes','No',...]` 로 FAIL | **오탐일 수 있습니다.** 이 evaluator 는 **trace level** 이라 한 시나리오 안에서도 서브에이전트 호출마다 채점됩니다. 실측 예: 6 trace 중 1건만 `No` → 게이트가 "전부 Yes" 라서 FAIL. 아래 해석 절 참고 |
| `agentcore run eval` 이 `contains spanIds that do not exist in the provided data` | **`Builtin.ToolSelectionAccuracy` 를 넣었을 때 발생합니다.** CLI 가 채점 대상으로 지목한 tool span 이 평가 요청에 담기지 않아 생기는 불일치입니다. **5분 더 기다려도 같은 spanId 가 그대로 나오므로 인덱싱 지연이 아닙니다** (실측 확인). ① 그 evaluator 를 빼고 `Helpfulness` / `GoalSuccessRate` 로 실행하거나, ② 도구 평가가 필요하면 4단계의 `Builtin.Trajectory*Match`(프로그램 채점·비용 0) 또는 `run-golden-eval.py`(span 을 직접 수집해 `sessionSpans` 로 전달)를 쓰세요 |
| `No such command 'run'` | 구 starter-toolkit 의 `agentcore` 가 PATH 에서 먼저 잡혔습니다. `which -a agentcore` 로 확인하고 `pip uninstall bedrock-agentcore-starter-toolkit` 하세요. requirements.txt 에서는 이미 제외했습니다 |
...
## ⚠️ evaluator level 차이 — FAIL 을 해석하는 법

같은 시나리오인데 evaluator 마다 결과 개수가 다릅니다. **level 이 다르기 때문**입니다.

| evaluator | level | 한 시나리오당 결과 수 | 실측 예 |
|---|---|---|---|
| `GoalSuccessRate` · `Trajectory*Match` | **Session** | 1건 | `Yes` / `1.00` |
| `Helpfulness` · `ToolSelectionAccuracy` · `Correctness` | **Trace** | trace 수만큼 | `['Yes','Yes','Yes','Yes','No','Yes']` |

실제 완주에서 나온 케이스입니다:

```
[evaluate] REC_Q07
  · Builtin.Helpfulness: 평균 1.00 (gate 0.5) ✅ PASS
  · Builtin.GoalSuccessRate: Yes ✅ PASS
  · Builtin.ToolSelectionAccuracy: ['Yes','Yes','Yes','Yes','No','Yes'] ❌ FAIL
  · Builtin.TrajectoryInOrderMatch: 평균 1.00 (gate 1.0) ✅ PASS
```

**세션 전체 관점(GoalSuccessRate)과 도구 순서(Trajectory)는 통과했는데 trace 하나가 `No`** 입니다. 우리 4-에이전트 구조는 한 요청에 Orchestrator + 서브에이전트가 각각 trace 를 만들어서, 그중 **도구를 부르지 않는 trace**(예: Summary 에이전트의 톤 정리 호출)가 `No` 로 채점될 수 있습니다.

> **판단 기준** — `GoalSuccessRate` 와 `Trajectory*Match` 가 통과했다면 **기능은 정상**입니다. `ToolSelectionAccuracy` 의 개별 `No` 는 회귀 신호가 아니라 멀티에이전트 구조의 특성일 수 있으니, `explanation` 을 열어 어느 trace 인지 확인하세요.

**게이트를 어떻게 둘까** — 현재 `DEFAULT_GATE` 는 "전부 Yes" 라 엄격합니다. 목적에 따라 조정하세요:

```python
# scripts/run-golden-eval.py 의 DEFAULT_GATE
"Builtin.ToolSelectionAccuracy": {"Yes", "SUCCESS"},   # 현재: 전부 Yes (엄격)
```

| 목적 | 권장 |
|---|---|
| **도구 선택 회귀를 정확히 잡기** | `expected_trajectory` + `Trajectory*Match` 를 주 게이트로. 프로그램적 채점이라 오탐이 없고 **비용 0** |
| 답변 품질 게이트 | `Helpfulness`(평균 임계값) + `GoalSuccessRate`(session level) |
| `ToolSelectionAccuracy` | **참고 지표로 두고 게이트에서 빼는 것**을 권합니다 — 멀티에이전트에서 trace 단위 오탐이 잦습니다 |

> 워크샵 기본 설정은 **모든 evaluator 를 게이트에 넣어** 이 차이를 체감하게 했습니다. 실무에서는 위 표대로 좁히는 게 맞습니다.
