...
> — [Ground truth evaluations](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/ground-truth-evaluations.html)
>
> API 로도 교차 확인됩니다 — `CreateOnlineEvaluationConfig` 에는 `evaluationReferenceInputs` 에 해당하는 필드가 아예 없습니다 (`rule` / `dataSourceConfig` / `evaluators` / `insights` / `clusteringConfig` 등). 즉 정답을 넘길 통로 자체가 없습니다.
>
...
| `agentcore run eval` 이 `contains spanIds that do not exist in the provided data` | CLI 가 수집한 span 집합과 evaluationTarget 으로 지정한 span 이 어긋난 경우입니다 (실측 발생). **워크샵 스크립트를 쓰세요** — `run-golden-eval.py` 는 span 을 직접 수집해 `sessionSpans` 로 넘기므로 이 문제가 없습니다. 실제 계정에서 4개 evaluator 전부 PASS 확인 |
