...
> **검증 완료** — `FileDatasetProvider("docs/eval/golden-set.json").get_dataset()` 로 로드해 20개 전부 `PredefinedScenario` 로 인식되는 것을 확인했습니다.

### 공식이 제시하는 두 가지 데이터셋 경로

같은 스키마를 쓰지만 보관 방식이 두 갈래입니다. **워크샵은 B** 를 씁니다.

| | A. CLI 데이터셋 리소스 | B. 로컬 파일 (이 워크샵) |
|---|---|---|
| 파일 | `agentcore/datasets/<name>.jsonl` (**JSONL** — 한 줄 = 한 시나리오) | `golden-set.json` (**단일 JSON** — `{"scenarios": [...]}`) |
| 등록 | `agentcore add dataset --schema-type AGENTCORE_EVALUATION_PREDEFINED_V1` → `agentcore deploy` | 없음 (파일만 있으면 됨) |
| 버전 관리 | 서비스가 관리 (`dataset publish-version`, 버전별 조회) | git |
| 로드 | `--dataset <name>` | `FileDatasetProvider(path)` 또는 직접 파싱 |
| 한도 | 요청당 예제 1,000개 · inline 5MB | 없음 |

> **왜 B 인가** — 골든셋을 **git 에 두고 코드와 함께 리뷰·버전 관리**하는 것이 회귀 테스트의 기본이고, PR 게이트(7단계)에 그대로 붙습니다. A 는 여러 팀이 데이터셋을 공유하거나 서비스측 버전 이력이 필요할 때 유리합니다.
>
> 형식만 다르고 **스키마는 동일**하므로 전환은 쉽습니다 — A 로 옮기려면 `scenarios` 배열을 한 줄씩 JSONL 로 풀어 쓰면 됩니다.

### assertion 을 어떻게 쓰라고 하나

공식 문서는 형식을 규정하지 않고 **자유 형식 자연어**라고 명시합니다. 대신 무엇을 검증할 수 있는지 축을 알려줍니다:

> *"assertions are free-form natural language — you can use them to assert on any aspect of agent behavior, such as **response tone, factual accuracy, safety compliance, or business logic**."*
> — [Ground truth evaluations](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/ground-truth-evaluations.html)

공식 예시는 대부분 도구 사용 검증(`"Agent used the calculator tool..."`)이지만, 위 문장이 그건 예시일 뿐이라고 밝히고 있습니다. 이 워크샵 골든셋은 네 축을 의도적으로 나눠 담았습니다:

| 축 | 시나리오 | assertion 예 |
|---|---|---|
| **factual accuracy** | `INFO_Q01`~`Q06` | "천기단 복합 성분과 함께 장뇌삼, 영지, 청아교 중 최소 두 가지를 언급해야 한다." |
| **business logic** | `REC_Q07`~`Q11`, `MIX_Q17`~`Q19` | "건성 피부용 에센스를 추천하고 추천한 상품의 재고 상태를 함께 안내해야 한다." |
| **safety compliance** | `SAFE_Q20` | "여드름 '치료', '완치' 같은 의료 효능 표현 없이 도움이 될 수 있다는 수준으로 답해야 한다." |
| **도구 선택** | `expected_trajectory` (19/20) | judge 가 아닌 `Trajectory*Match` 가 프로그램적으로 채점 |

**작성할 때 지킨 원칙** — 실무에서 골든셋을 만들 때 그대로 쓰시면 됩니다:
...
3. **실제 데이터에서 근거를 가져온다** — 성분·가격·사용법을 상품 데이터(29종)에서 그대로 인용했습니다. 지어낸 정답으로 채점하면 회귀 테스트가 아니라 오답 고정입니다.
4. **도구 검증은 `expected_trajectory` 로 넘긴다** — assertion 으로 "recommend_tool 을 써야 한다" 를 쓰면 judge LLM 이 토큰을 쓰지만, `Trajectory*Match` 는 **비용 0** 입니다.
...
### ground truth 필드의 scope — 헷갈리기 쉬운 지점

공식 [Ground truth evaluations](https://docs.aws.amazon.com/bedrock-agentcore/latest/devguide/ground-truth-evaluations.html) 문서 기준, 필드마다 적용 범위가 다릅니다.

| 필드 | scope | 의미 |
|---|---|---|
| `assertions` | **Session** | 세션 전체에 대해 1건 채점 |
| `expectedTrajectory` | **Session** | 세션 전체의 도구 호출 시퀀스 1건 |
| `expectedResponse` | **Trace** | 특정 turn 의 답변. 매핑 규칙이 **호출 계층마다 다릅니다** — 아래 표 참고 |

**`expected_response` 매핑 — 두 공식 문서를 구분해 읽어야 합니다**
...
러너는 turns 를 순서대로 실행하므로 위치 기반 매핑이 성립하고, API 를 직접 부를 때는 어느 trace 인지 알려줄 방법이 `traceId` 뿐입니다. **워크샵 스크립트는 후자**이므로, 멀티턴 시나리오에서 턴마다 정답을 채점하려면 turn 별 `traceId` 를 찾아 reference input 을 나눠야 합니다. (현재 골든셋은 전부 단일 턴 + `expected_response` 미사용이라 해당되지 않습니다.)

> 멀티턴 시나리오에서 턴마다 정답을 주려면 turn 별 `traceId` 를 찾아 reference input 을 여러 개로 나눠야 합니다. 이 워크샵 골든셋은 전부 단일 턴이라 해당되지 않습니다.

**evaluator 가 안 쓰는 필드는 무시됩니다 — 에러가 아닙니다.** 응답의 `ignoredReferenceInputFields` 에 어떤 필드가 안 쓰였는지 보고됩니다. 그래서 reference input 을 한 번 만들어 여러 evaluator 에 그대로 재사용할 수 있습니다.

### trajectory evaluator 3종 비교 — 비용이 0 입니다

세 evaluator 는 모두 **programmatic scoring** 이라 LLM 을 호출하지 않습니다. 즉 **토큰 비용이 0** 입니다 (공식 문서: *"programmatic scoring (no LLM calls, so token usage is zero)"*).

| evaluator | 판정 규칙 | 기대 `[qa_tool, recommend_tool]` 에 대한 실제 `[qa_tool, x, recommend_tool]` |
|---|---|---|
| `TrajectoryExactOrderMatch` | 정확히 일치 — 순서 같고 **추가 도구 불허** | ❌ FAIL |
| `TrajectoryInOrderMatch` | 기대 도구가 **순서대로** 등장, 사이에 추가 도구 허용 | ✅ PASS |
| `TrajectoryAnyOrderMatch` | 기대 도구가 **모두 존재**, 순서 무관 | ✅ PASS |

> 이 워크샵은 `TrajectoryInOrderMatch` 를 씁니다. 에이전트가 재시도나 추가 조회를 하는 건 정상이므로 `Exact` 는 너무 엄격하고, 순서가 의미를 갖는 경우(검색 → 추천)가 있어 `AnyOrder` 는 너무 느슨합니다.
>
> **비용이 0 이라는 점을 활용하세요** — 골든셋 규모를 키울 때 LLM-as-Judge evaluator 는 토큰 quota(분당 200,000)에 걸리지만 trajectory 계열은 무료입니다. 도구 선택 회귀만 넓게 잡고 싶다면 trajectory evaluator 중심으로 구성하는 게 경제적입니다.
