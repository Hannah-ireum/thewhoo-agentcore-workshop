...
>
> 이 워크샵은 데이터와 질문이 **모두 한국어**라 일관성이 있어서 실측 score 0.70-0.72 가 나옵니다. 다만 실서비스에서 다국어 상담을 계획한다면 임베딩 모델 선택을 다시 검토해야 합니다 — 언어별로 KB 를 나누거나, 다국어에 더 강한 임베딩 모델(예: Cohere Embed Multilingual)을 고려하세요. Lab 0 의 `embeddingModelArn` 한 줄만 바꾸면 되지만, **KB 생성 후에는 변경 불가**라 재생성이 필요합니다.
