...
echo "Lab 5 에서 AgentCore CLI 로 배포합니다:"
echo "  npm install -g @aws/agentcore"
echo "  agentcore create --name ThewhooChat --framework Strands --protocol HTTP \\"
echo "    --model-provider Bedrock --memory none --build CodeZip"
echo "  python3 scripts/set-agentcore-config.py   # src/ 를 가리키게 설정 + envVars 주입"
echo "  agentcore deploy --dry-run -y  &&  agentcore deploy -y"
